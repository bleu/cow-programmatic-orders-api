...
| `schema/tables.ts` | Table definitions: `conditionalOrder`, `orders` |
| `schema/relations.ts` | Drizzle relations: `conditionalOrder → many(orders)`, `orders → one(conditionalOrder)` |
...
| `src/application/handlers/composableCow.ts` | `ConditionalOrderCreated` event handler — computes hash, inserts `conditionalOrder` row |
...
| Column | Type | Notes |
|--------|------|-------|
| `id` | text PK | Ponder event ID |
| `owner` | text NOT NULL | Indexed |
| `handler` | text NOT NULL | Indexed |
| `salt` | text NOT NULL | |
| `staticInput` | text NOT NULL | |
| `hash` | text NOT NULL | keccak256 of ABI-encoded params |
| `txHash` | text NOT NULL | |

**`orders`** (placeholder for future S2.x)
| Column | Type | Notes |
|--------|------|-------|
| `orderUid` | text PK | |
| `conditionalOrderId` | text NOT NULL | FK → conditionalOrder.id |

### Relations (`schema/relations.ts`)

- `conditionalOrder` has many `orders`
- `orders` belongs to one `conditionalOrder`
...
```graphql
{ conditionalOrders(limit: 5) { items { id owner handler hash } } }
```
