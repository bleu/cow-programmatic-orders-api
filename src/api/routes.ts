...
export const syncProgressRoute = createRoute({
  ...
  description:
    "Returns the indexer's historical backfill progress per chain: total blocks to process, blocks already processed, percentage complete, and whether the chain is in realtime mode. Reads from Ponder's built-in Prometheus metrics. During initial sync, historicalSyncProgressPct will rise from 0 to 100 and isComplete will flip to true once the chain is fully caught up.",
  responses: {
    200: {
