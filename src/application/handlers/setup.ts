...
ponder.on("ComposableCow:setup", async ({ context }) => {
  ...
  await context.db.sql.execute(sql`ALTER TABLE cow_cache.order_uid_cache ADD COLUMN IF NOT EXISTS executed_fee_amount TEXT`);
  ...
  `);
  await context.db.sql.execute(sql`ALTER TABLE cow_cache.composable_order ADD COLUMN IF NOT EXISTS executed_fee_amount TEXT`);
  await context.db.sql.execute(sql`
