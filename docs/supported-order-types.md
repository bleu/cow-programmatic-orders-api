...
Most handler addresses are identical across all active chains (CREATE2 deployments — see `src/utils/order-types.ts` for chain-specific overrides). Chains defined but not yet enabled may not have their handler mappings registered.
...
- **`FlashLoanOrderBackfiller`** fires once at go-live and bulk-drains the entire historical backlog in bounded slices — so the deploy's incomplete-data window after promotion is ~one firing, not hours.
...
`Unknown` and `CowAmmConstantProduct` generators are recorded but their discrete-order history is not owner-backfilled (`OWNER_BACKFILL_EXCLUDED` in `src/utils/order-types.ts`) — unknown handlers are unsupported, and CoW AMM migrated out of ComposableCoW.
